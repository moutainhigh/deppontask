...
import com.github.ltsopensource.store.jdbc.exception.SuspendJobExistException;
...
public class JobReceiver {
    ...
    public void receive(JobSubmitRequest request) throws JobReceiveException {
        ...
        JobReceiveException exception = null;
        SuspendJobExistException exceptionSuspend = null;
        for (Job job : jobs) {
            try {
                addToQueue(job, request);
            } catch (Exception e) {
            	/**
                 * 增加暂停任务队列中已存在相同taskId异常逻辑
                 * @author chenhaitao
                 */
            	/*Class<? extends Exception> class1 = e.getClass();
            	if(class1.getName().equals("com.github.ltsopensource.store.jdbc.exception.SuspendJobExistException")){
            		exceptionSuspend = new SuspendJobExistException();
            		throw exceptionSuspend;
                }*/
                if (exception == null) {
                    exception = new JobReceiveException(e);
                    //((JobReceiveException) exception).addJob(job);
                }
    ...
    private JobPo addToQueue(Job job, JobSubmitRequest request) throws SuspendJobExistException {
        ...
        } catch (SuspendJobExistException e) {
        	//如果暂停任务队列中已经相同taskId的任务
        	 LOGGER.info("Job already exist in suspend_job_queue. nodeGroup={}, {}", request.getNodeGroup(), job);
        	 job.setExistInSuspend(true);
        	 code = BizLogCode.DUP_FAILED;
		} finally {
            if (success) {
     ...
     * 添加任务
     */
    private void addJob(Job job, JobPo jobPo) throws DupEntryException,SuspendJobExistException {
        if (job.isCron()) {
            addCronJob(job,jobPo);
     ...
     * 添加Cron 任务
     */
    private void addCronJob(Job job, JobPo jobPo) throws DupEntryException,SuspendJobExistException {
        ...
        if (nextTriggerTime != null) {
            ...
            /**
        	 *  增加逻辑:如果暂停任务队列中有相同id的taskId的话,则不可添加此队列(提醒用户此不可使用重复的taskId)
        	 *  author:chenhaitao 333930
        	 */
            if(appContext.getSuspendJobQueue().getJob(jobPo.getTaskTrackerNodeGroup(),jobPo.getTaskId()) == null){
            	appContext.getCronJobQueue().add(jobPo);
            }
            else{
            	job.setExistInSuspend(true);
            	throw new SuspendJobExistException();
            }
