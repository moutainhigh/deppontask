...
import com.deppon.lts.result.DepponLtsResult;
...
//			return DepponLtsResult.build("TAOTAO-CodisStorage：get value不允许key为null");
//		}
//		if(!poolInited.get()){
//			return DepponLtsResult.build("TAOTAO-CodisStorage：jedis pool没有初始化");
//		}
		Jedis jedis = null;
		String result;
		try {
...
//				return DepponLtsResult.build("返回的value为空");
//			}
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			return DepponLtsResult.build("jedis连接出现异常");
		}finally {
			if(jedis != null){
				jedis.close();
			}
...
//			return DepponLtsResult.build("TAOTAO-CodisStorage：get value不允许key或者hkey为null");
//		}
//		if(!poolInited.get()){
//			return DepponLtsResult.build("TAOTAO-CodisStorage：jedis pool没有初始化");
//		}
